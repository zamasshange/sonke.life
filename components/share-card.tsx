...
export function ShareCard({ breakdown, monthlyIncome, country }: ShareCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={cardRef}
        className="bg-white p-8 text-black shadow-lg relative overflow-hidden"
      ...
      >
        ...
        <div className="border-b-2 border-black pb-6 mb-6">
          <div className="flex items-start justify-between gap-8">
            <div className="flex items-center gap-5">
              <div className="grid h-20 w-20 place-items-center overflow-hidden rounded-sm bg-black">
                <img src="/images/favicon.png" alt="Sonke mark" className="h-full w-full object-contain" />
              </div>
              <div>
                <h3 className="text-4xl font-black uppercase tracking-tight text-black">Sonke</h3>
                <p className="text-xs font-bold uppercase tracking-[0.28em] text-black/60 mt-1">
                  Financial Reality Statement
                </p>
                <div className="flex items-center gap-2 mt-3">
                  <span className="px-2 py-1 bg-primary text-white text-[10px] font-bold uppercase tracking-wider">
                    {flag} {name}
                  </span>
                  <span className="px-2 py-1 bg-black/10 text-black/60 text-[10px] font-bold uppercase tracking-wider">
                    {currency}
                  </span>
                </div>
              </div>
            ...
            <div className="text-right">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-black/40">Statement Date</p>
              <p className="mt-1 font-mono font-bold text-black text-lg">{generatedDate}</p>
              <p className="mt-4 text-[10px] font-black uppercase tracking-[0.2em] text-black/40">Document ID</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 p-5 rounded-sm">
            <div className="flex items-center gap-2 mb-3">
              <TrendingUp className="h-4 w-4 text-emerald-600" />
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Monthly Income</p>
            </div>
            <p className="font-mono text-3xl font-black text-emerald-800">{formatCurrency(monthlyIncome)}</p>
            <p className="text-[10px] text-emerald-600 mt-2">Take-home pay</p>
          </div>
          <div className="bg-gradient-to-br from-red-50 to-red-100 border border-red-200 p-5 rounded-sm">
            <div className="flex items-center gap-2 mb-3">
              <TrendingDown className="h-4 w-4 text-red-600" />
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-red-700">Total Expenses</p>
            </div>
            <p className="font-mono text-3xl font-black text-red-800">{formatCurrency(breakdown.total)}</p>
            <p className="text-[10px] text-red-600 mt-2">{statementItems.length} expense categories</p>
          </div>
          <div className={`p-5 rounded-sm ${isPositive ? "bg-gradient-to-br from-emerald-500 to-emerald-600 text-white" : "bg-gradient-to-br from-primary to-orange-500 text-white"}`}>
            <div className="flex items-center gap-2 mb-3">
              {isPositive ? <PiggyBank className="h-4 w-4 text-white/80" /> : <Target className="h-4 w-4 text-white/80" />}
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/80">
                {isPositive ? "Surplus" : "Shortfall"}
              </p>
            </div>
            <p className="font-mono text-3xl font-black">{formatCurrency(difference)}</p>
            <p className="text-[10px] text-white/80 mt-2">
              {isPositive ? `${percentageUsed}% income used` : `${100 - percentageUsed}% shortfall`}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 p-4 bg-gray-50 rounded-sm border border-gray-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-black/60">Income Utilization</p>
            <p className="text-xs font-bold font-mono text-black">{percentageUsed}%</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-blue-50 border border-blue-200 p-5 rounded-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Living Costs</p>
            </div>
            <p className="font-mono text-3xl font-black text-blue-800">{formatCurrency(breakdown.totalLiving)}</p>
          ...
          <div className="bg-purple-50 border border-purple-200 p-5 rounded-sm">
            <div className="flex items-center gap-2 mb-3">
              <Wifi className="h-5 w-5 text-purple-600" />
              <p className="text-xs font-black uppercase tracking-[0.18em] text-purple-700">Digital Costs</p>
            </div>
            <p className="font-mono text-3xl font-black text-purple-800">{formatCurrency(breakdown.totalDigital)}</p>
            <div className="mt-3 flex items-center gap-2">
        ...
        {/* Motivational quote */}
        <div className="mb-6 p-4 bg-gradient-to-r from-primary/10 via-orange-500/10 to-amber-500/10 border border-primary/20 rounded-sm">
          <p className="text-sm italic text-black/70">"{motivationalQuotes[activeQuote]}"</p>
        </div>

        {/* Yearly projection */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="p-4 border border-black/10 rounded-sm text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-black/50">Weekly Average</p>
            <p className="font-mono text-xl font-black text-black mt-2">{formatCurrency(breakdown.total / 4.33)}</p>
          </div>
          <div className="p-4 bg-black text-white rounded-sm text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/70">Yearly Total</p>
            <p className="font-mono text-xl font-black text-white mt-2">{formatCurrency(breakdown.total * 12)}</p>
          </div>
          <div className="p-4 border border-black/10 rounded-sm text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-black/50">Daily Average</p>
            <p className="font-mono text-xl font-black text-black mt-2">{formatCurrency(breakdown.total / 30)}</p>
          </div>
        </div>
